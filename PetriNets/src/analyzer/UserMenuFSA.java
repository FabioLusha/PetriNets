...
public enum UserMenuFSA {
	...
	ADD_ELEMENT {
		...
		public UserMenuFSA stepNext() {
			...
			do {
				...
				// TO-DO messaggio in base al perché torna false
	...
	CREATE_PETRI_NET {
		public UserMenuFSA stepNext() {
			...
			if (netMap.isEmpty()) {
				System.out.println("Non vi � alcuna rete presente per poter definire una rete di Petri!");
				return MAIN_MENU;
			} else {
	...
	CHANGE_PLACE_VALUE {
		public UserMenuFSA stepNext() {
			...
			if (temporaryPetriNet.getMarcmap().keySet().contains(new Place(choice))) {
				...
				System.out.println("Non � presente alcun posto con questo nome!");
				return ADD_VALUES_TO_PETRI_NET;
			}
		}
	},
